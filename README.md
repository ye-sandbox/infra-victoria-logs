...
2. **Docker Compose Hardening:** Docker socket (`/var/run/docker.sock`) mounted strictly read-only (`:ro`), immutable root filesystem (`read_only: true`) with ephemeral volatile mounts (`tmpfs: [/tmp]`), privilege escalation blocked (`security_opt: ["no-new-privileges:true"]`), hard memory limits enforced (`<= 80M` and `<= 60M`, total `<= 150M`), log loop prevention (`exclude_containers: ["vector"]`), and active healthchecks.
3. **Network Exposure:** Flags unauthenticated public ports listening on `0.0.0.0`.
4. **Runtime Inspection:** Verifies that active kernel cgroups match configured limits, `/var/run/docker.sock` is read-only, running containers enforce read-only root filesystems, and `no-new-privileges` is applied in the kernel (`HostConfig.SecurityOpt`).
